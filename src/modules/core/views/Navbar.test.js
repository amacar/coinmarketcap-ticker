...
describe('active classname on right element', () => {
  ...
  it('init path, active on home (<li>)', async () => {
	...
	expect(wrapper.find('Link').at(0).childAt(0).hasClass('active')).toEqual(true);
	expect(wrapper.find('Link').at(1).childAt(0).hasClass('active')).toEqual(false);
  });
  
  it('settings path, active on settings (<li>)', async () => {
    const wrapper = createWrapper(settingsPath);
	
	expect(wrapper.find('Link').at(0).childAt(0).hasClass('active')).toEqual(false);
	expect(wrapper.find('Link').at(1).childAt(0).hasClass('active')).toEqual(true);
  });
